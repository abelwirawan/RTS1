...
public class TopCamera : MonoBehaviour
{
    ...
    Vector2 CameraAngle = new Vector2(45f, 45f); // Sudut RTS klasik
    ...
    void Start()
    {
        ...
        transform.rotation = Quaternion.Euler(currentAngle.x, currentAngle.y, 0f);
    ...
    void Update()
    {
        // Rotasi kamera dengan klik tengah (lebih halus dan natural)
        ...
        if (isDragging)
        {
            ...
            currentAngle.y += delta.x * sensitivity; // rotasi horizontal
            currentAngle.x -= delta.y * sensitivity; // rotasi vertikal
            currentAngle.x = Mathf.Clamp(currentAngle.x, 30f, 65f); // Sudut RTS nyaman
            transform.rotation = Quaternion.Euler(currentAngle.x, currentAngle.y, 0f);
            lastMousePosition = Input.mousePosition;
        }

        // Mouse drag (pan) dengan klik tengah
        if (Input.GetMouseButtonDown(2)) // 2 = klik tengah
        {
            isDragging = true;
            lastMousePosition = Input.mousePosition;
        }
        ...
        {
            Vector3 delta = Input.mousePosition - lastMousePosition;
            // Geser kamera secara horizontal dan vertical
            MouseMove(new Vector2(-delta.x, -delta.y) * 0.1f); // 0.1f = sensitivitas, bisa diatur
            lastMousePosition = Input.mousePosition;
        }

        float edgeSize = 10f; // pixel dari tepi layar
        Vector2 move = Vector2.zero;
        if (Input.mousePosition.x < edgeSize) move.x = -1;
        else if (Input.mousePosition.x > Screen.width - edgeSize) move.x = 1;
        if (Input.mousePosition.y < edgeSize) move.y = -1;
        else if (Input.mousePosition.y > Screen.height - edgeSize) move.y = 1;
